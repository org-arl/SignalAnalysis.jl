...
Base.time(s::AbstractArray; fs=deffs) = float(0:size(s,1)-1) ./ fs

"Get total signal energy."
energy(s::AbstractVector; fs=deffs) = sum(abs2, s)/fs
energy(s::AbstractMatrix; fs=deffs) = dropdims(sum(abs2, s; dims=1); dims=1)./fs
...
function ifreq(s; fs=deffs)
    s1 = analytic(s)
    f1 = fs/(2π) * diff(unwrap(angle.(s1); dims=1); dims=1)
